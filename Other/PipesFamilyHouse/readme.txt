...
子プロセス（Child.exe）にデバッガを
アタッチ（Debug）する場合は下記を参照。
...
　　コンソールへの入出力（シェル）が必要なアプリケーションで利用。
...
　　
・リダイレクト：標準入出力の付け替えを指す。
　（上記のハンドルをコンソール → ファイル、匿名パイプなどと変更）
　
　
＜参考（C言語での実装）＞
...
　　　hOutputReadTmp（親プロセスで使うTmp）、hOutputWrite（子プロセスに渡す）
　　　
　　・子プロセスからデータを入力可能な匿名パイプ（継承可能）
　　　hInputRead（子プロセスに渡す）、hInputWriteTmp（親プロセスで使うTmp）
　　　
　　・親プロセスで使う匿名パイプ・ハンドル
