        ...
        private static Shard CreateOrGetEmptyShard(RangeShardMap<int> shardMap)
        {
            ...
            if (shard == null)
            {
                ...
                SqlDatabaseUtils.ExecuteSqlScript(
                    MultiShardConfiguration.ShardMapManagerServerName, databaseName, InitializeShardScriptFile);

                // Add it to the shard map
                ShardLocation shardLocation = new ShardLocation(MultiShardConfiguration.ShardMapManagerServerName, databaseName);
